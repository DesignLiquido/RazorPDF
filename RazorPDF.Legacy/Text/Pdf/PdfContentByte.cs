...
using RazorPDF.Legacy.Text.Exceptions;
using RazorPDF.Legacy.Text.Pdf;
using RazorPDF.Legacy.Text.Pdf.Intern;
        ...
        */
        ...
        public void Rectangle(Rectangle rectangle) {
            ...
            if (rectangle.UseVariableBorders) {
                ...
                if (rectangle.BorderWidth != RazorPDF.Legacy.Text.Rectangle.UNDEFINED) {
                ...
                if (rectangle.HasBorder(RazorPDF.Legacy.Text.Rectangle.BOX)) {
                    this.Rectangle(x1, y1, x2 - x1, y2 - y1);
                }
                    // if the border isn't a rectangle, the different sides are added apart
                else {
                    if (rectangle.HasBorder(RazorPDF.Legacy.Text.Rectangle.RIGHT_BORDER)) {
                        MoveTo(x2, y1);
                        LineTo(x2, y2);
                    }
                    if (rectangle.HasBorder(RazorPDF.Legacy.Text.Rectangle.LEFT_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x1, y2);
                    }
                    if (rectangle.HasBorder(RazorPDF.Legacy.Text.Rectangle.BOTTOM_BORDER)) {
                        MoveTo(x1, y1);
                        LineTo(x2, y1);
                    }
                    if (rectangle.HasBorder(RazorPDF.Legacy.Text.Rectangle.TOP_BORDER)) {
                        MoveTo(x1, y2);
                        LineTo(x2, y2);
                    }
                }
