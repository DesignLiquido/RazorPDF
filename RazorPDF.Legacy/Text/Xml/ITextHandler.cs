...
using RazorPDF.Legacy.Text.Pdf.Draw;
using RazorPDF.Legacy.Text.Xml.SimpleParser;
using RazorPDF.Legacy.Text.Html;
using RazorPDF.Legacy.Text.Factories;
...
namespace RazorPDF.Legacy.Text.Xml
{
    ...
    public class ITextHandler : ParserBase
    {
        ...
        */
        ...
        public void HandleStartingTags(String name, TagProperties attributes)
        {
            ...
            if (IsDocumentRoot(name))
            {
                ...
                foreach (string key in attributes.Keys)
                {
                    ...
                    if (Util.EqualsIgnoreCase(ElementTags.LEFT, key))
                        leftMargin = float.Parse(value, NumberFormatInfo.InvariantInfo);
                    if (Util.EqualsIgnoreCase(ElementTags.RIGHT, key))
                        rightMargin = float.Parse(value, NumberFormatInfo.InvariantInfo);
                    if (Util.EqualsIgnoreCase(ElementTags.TOP, key))
                        topMargin = float.Parse(value, NumberFormatInfo.InvariantInfo);
                    if (Util.EqualsIgnoreCase(ElementTags.BOTTOM, key))
                        bottomMargin = float.Parse(value, NumberFormatInfo.InvariantInfo);
